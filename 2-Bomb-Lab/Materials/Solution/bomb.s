...
0000000000400efc <phase_2>:
  400efc:	55                   	push   %rbp
  400efd:	53                   	push   %rbx
  400efe:	48 83 ec 28          	sub    $0x28,%rsp   // 分配当前栈帧40字节
  //1.读取数字
  400f02:	48 89 e6             	mov    %rsp,%rsi    //将当前栈的栈顶指针作为参数传递给read_six_numbers
  400f05:	e8 52 05 00 00       	callq  40145c <read_six_numbers>  //读取数字
  //2.判断读取的第一个数字 == 1
  400f0a:	83 3c 24 01          	cmpl   $0x1,(%rsp)  // 将读取的第一个数字(num1) 和 1进行比较 
  400f0e:	74 20                	je     400f30 <phase_2+0x34>   // 如果num1 == 1就跳转到phase_2+0x34
  400f10:	e8 25 05 00 00       	callq  40143a <explode_bomb>   // 如果num1 != 1 就爆炸 
  //3.循环判断后续的数字
  400f15:	eb 19                	jmp    400f30 <phase_2+0x34>
  400f17:	8b 43 fc             	mov    -0x4(%rbx),%eax   //  获取上一个数字 
  400f1a:	01 c0                	add    %eax,%eax         //  加倍，即乘2
  400f1c:	39 03                	cmp    %eax,(%rbx)       //  判断当前数字 是不是  上一个数字的两倍
  400f1e:	74 05                	je     400f25 <phase_2+0x29>
  400f20:	e8 15 05 00 00       	callq  40143a <explode_bomb>  //如果不是就爆炸 
  400f25:	48 83 c3 04          	add    $0x4,%rbx         //  如果是 就指针后移 
  400f29:	48 39 eb             	cmp    %rbp,%rbx
  400f2c:	75 e9                	jne    400f17 <phase_2+0x1b>   //  判断指针是不是 指向了结尾
  400f2e:	eb 0c                	jmp    400f3c <phase_2+0x40>
  400f30:	48 8d 5c 24 04       	lea    0x4(%rsp),%rbx    // 指针指向下一个数字 
  400f35:	48 8d 6c 24 18       	lea    0x18(%rsp),%rbp   // 相当于a+6 ,a是存储6个数字的首地址，a+5是最后一个数字的地址
...
0000000000400f43 <phase_3>:
  ...
  //进行数据的解析读取
  400f47:	48 8d 4c 24 0c       	lea    0xc(%rsp),%rcx  // num2的地址 %rsp+0xc
  400f4c:	48 8d 54 24 08       	lea    0x8(%rsp),%rdx  // num1的地址 %rsp+0x8 从中可以看出该数字占4字节（0xc -0x8）
  400f51:	be cf 25 40 00       	mov    $0x4025cf,%esi  // 通过x /s 0x4025cf 可以发现是"%d %d" 意味着输入的数字是两个
  400f56:	b8 00 00 00 00       	mov    $0x0,%eax       // %eax用于存放sscanf的返回值，初始化为0
  400f5b:	e8 90 fc ff ff       	callq  400bf0 <__isoc99_sscanf@plt>  // 调用sscanf  
  //判断输入数字的数目
  400f60:	83 f8 01             	cmp    $0x1,%eax  // 要求输入的数字 数目大于1
  400f63:	7f 05                	jg     400f6a <phase_3+0x27>
  400f65:	e8 d0 04 00 00       	callq  40143a <explode_bomb>   // 如果输入的数字数目 <= 1 就爆炸 
  400f6a:	83 7c 24 08 07       	cmpl   $0x7,0x8(%rsp)   // 要求 num1 <= 7  
  400f6f:	77 3c                	ja     400fad <phase_3+0x6a>  // 如果num1 > 7 就爆炸
  //进行switch的选择跳转
  400f71:	8b 44 24 08          	mov    0x8(%rsp),%eax  // 获取第一个数字num1 
  400f75:	ff 24 c5 70 24 40 00 	jmpq   *0x402470(,%rax,8)  // 跳转  0x402470是switch跳转表的首地址
  400f7c:	b8 cf 00 00 00       	mov    $0xcf,%eax   // 当num1=0时跳转到这  %eax = 0xcf
  400f81:	eb 3b                	jmp    400fbe <phase_3+0x7b>  // 进行无条件跳转
  400f83:	b8 c3 02 00 00       	mov    $0x2c3,%eax  // 当num1=2时跳转到这 %eax = 0x2c3 
  400f88:	eb 34                	jmp    400fbe <phase_3+0x7b>
  400f8a:	b8 00 01 00 00       	mov    $0x100,%eax  // 当num1=3时跳转到这 %eax = 0x100
  400f8f:	eb 2d                	jmp    400fbe <phase_3+0x7b>
  400f91:	b8 85 01 00 00       	mov    $0x185,%eax  // 当num1=4时跳转到这 %eax = 0x185
  400f96:	eb 26                	jmp    400fbe <phase_3+0x7b>
  400f98:	b8 ce 00 00 00       	mov    $0xce,%eax   // 当num1=5时跳转到这 %eax = 0xce
  400f9d:	eb 1f                	jmp    400fbe <phase_3+0x7b>
  400f9f:	b8 aa 02 00 00       	mov    $0x2aa,%eax  // 当num1=6时跳转到这 %eax = 0x2aa
  400fa4:	eb 18                	jmp    400fbe <phase_3+0x7b>
  400fa6:	b8 47 01 00 00       	mov    $0x147,%eax  // 当num1=7时跳转到这 %eax = 0x147
  400fab:	eb 11                	jmp    400fbe <phase_3+0x7b>
  400fad:	e8 88 04 00 00       	callq  40143a <explode_bomb>  // 其它情况就爆炸
  400fb2:	b8 00 00 00 00       	mov    $0x0,%eax    // %eax = 0 
  400fb7:	eb 05                	jmp    400fbe <phase_3+0x7b> 
  400fb9:	b8 37 01 00 00       	mov    $0x137,%eax   // 当num1=1时跳转到这 %eax = 0x137
  400fbe:	3b 44 24 0c          	cmp    0xc(%rsp),%eax  // 判断%eax 和 num2是否相等
  400fc2:	74 05                	je     400fc9 <phase_3+0x86>  // %eax== num2 就拆炸弹成功
  400fc4:	e8 71 04 00 00       	callq  40143a <explode_bomb>  // %eax != num2 就爆炸 
...
//phase_3的c代码
 void phase_3(const char* input){
   int num1, num2;
   int len = sscanf(input, "%d %d", &num1, &num2);
   if(len <= 1) explode_bomb();
   int ret = 0;
   switch(num1){
     case 0:
       ret = 0xcf;
...
0000000000400fce <func4>:
  ...
  //1. 执行取两个数的中位数的操作
  400fd2:	89 d0                	mov    %edx,%eax  // 最开始 %edx中的内容是0xe 
  400fd4:	29 f0                	sub    %esi,%eax  // 最开始 %esi中的内容是0 
  400fd6:	89 c1                	mov    %eax,%ecx
  400fd8:	c1 e9 1f             	shr    $0x1f,%ecx // 逻辑右移 0x1f(十进制:31) 就是判断符号位是不是1，用于判断正负
  400fdb:	01 c8                	add    %ecx,%eax  // %ecx = %eax + 1 或者 %ecx = %eax + 0
  400fdd:	d1 f8                	sar    %eax       // 进行算数右移 相当于除2
  400fdf:	8d 0c 30             	lea    (%rax,%rsi,1),%ecx  // 进行 %ecx = %rax + %rsi*1
  //2. 进行递归调用
  400fe2:	39 f9                	cmp    %edi,%ecx  // %ecx 和 %edi 比较  最开始%edi是 num1
  400fe4:	7e 0c                	jle    400ff2 <func4+0x24>
  400fe6:	8d 51 ff             	lea    -0x1(%rcx),%edx  // 如果%ecx > %edi 就执行这个地方  %edx = %ecx - 1
  400fe9:	e8 e0 ff ff ff       	callq  400fce <func4>   // 递归调用
  400fee:	01 c0                	add    %eax,%eax        // func4的返回结果存在%eax中 将返回结果乘2
  400ff0:	eb 15                	jmp    401007 <func4+0x39>
  400ff2:	b8 00 00 00 00       	mov    $0x0,%eax        // 如果%ecx <= %edi 就执行这个地方  %eax = 0
  400ff7:	39 f9                	cmp    %edi,%ecx        // 比较%ecx 和 %edi
  400ff9:	7d 0c                	jge    401007 <func4+0x39>  // 如果%ecx == %edi 就直接返回 0;
  400ffb:	8d 71 01             	lea    0x1(%rcx),%esi   // 如果%ecx < %edi 就执行这个地方  %esi = %rcx + 1
...
// func4的c代码
// 注意一下的是 num in %edi, l in %esi, r in %rdx
 int func4(int num, int l, int r){
   // 获取l 和 r 的中位数
   int mid = l +(r - l)/2;
 ...
 // 要求func4的返回结果=0 就不会爆炸 最简单就是输入num = 7
...
000000000040100c <phase_4>:
  ...
  //1.数据的读取与解析
  401010:	48 8d 4c 24 0c       	lea    0xc(%rsp),%rcx  // num2 的地址 
  401015:	48 8d 54 24 08       	lea    0x8(%rsp),%rdx  // num1 的地址 
  40101a:	be cf 25 40 00       	mov    $0x4025cf,%esi  // 内存地址0x4025cf 中存放了 "%d %d"
  40101f:	b8 00 00 00 00       	mov    $0x0,%eax
  401024:	e8 c7 fb ff ff       	callq  400bf0 <__isoc99_sscanf@plt>  // 调用sscanf
  //2. 输入数据判断
  401029:	83 f8 02             	cmp    $0x2,%eax  // 要求输入两个数字 
  40102c:	75 07                	jne    401035 <phase_4+0x29>  // 如果不是两个数字 就爆炸
  40102e:	83 7c 24 08 0e       	cmpl   $0xe,0x8(%rsp)   // num1 <= 0xe (十进制：14)
  401033:	76 05                	jbe    40103a <phase_4+0x2e>  // 要求第一个数字 <= 14
  401035:	e8 00 04 00 00       	callq  40143a <explode_bomb>
  //3. 调用func4函数
  40103a:	ba 0e 00 00 00       	mov    $0xe,%edx   // %edx = 0xe   作为func4()的第三个参数
  40103f:	be 00 00 00 00       	mov    $0x0,%esi   // %esi = 0x0   作为func4()的第二个参数
  401044:	8b 7c 24 08          	mov    0x8(%rsp),%edi  // %edi = num1  作为func4()的第一个参数
  401048:	e8 81 ff ff ff       	callq  400fce <func4>  // 调用fun4函数
  //4. func4函数返回结果判断
  40104d:	85 c0                	test   %eax,%eax  // 判断 func4的返回值是不是0
  40104f:	75 07                	jne    401058 <phase_4+0x4c>  // 如果func4的返回值不为0就 爆炸
  401051:	83 7c 24 0c 00       	cmpl   $0x0,0xc(%rsp)  // 判断第二个数字是不是0
  ...
  void phase_4(const char* input){
   ...
   if(num1 > 14) explode_bomb();
   
   // 重点是fun4函数
   int ret = fun4(num1, 0, 14);
   if(ret != 0) explode_bomb();
   if(num2 != 0) explode_bomb();
   return;
 }
...
0000000000401062 <phase_5>:
  ...
  40107a:	e8 9c 02 00 00       	callq  40131b <string_length>  // 计算输入字符串的长度
  40107f:	83 f8 06             	cmp    $0x6,%eax   // 要求输入字符串的长度为6
  401082:	74 4e                	je     4010d2 <phase_5+0x70>
  401084:	e8 b1 03 00 00       	callq  40143a <explode_bomb>  // 不等于6就爆炸
  //2. 转换字符串
  401089:	eb 47                	jmp    4010d2 <phase_5+0x70>
  40108b:	0f b6 0c 03          	movzbl (%rbx,%rax,1),%ecx  // 将字符串的第%rax个字符转化为 对应的ascii码值
  40108f:	88 0c 24             	mov    %cl,(%rsp)
  401092:	48 8b 14 24          	mov    (%rsp),%rdx
  401096:	83 e2 0f             	and    $0xf,%edx  // ascii码值对16取余数  %edx = %edx % 16
  401099:	0f b6 92 b0 24 40 00 	movzbl 0x4024b0(%rdx),%edx  // 读取内存地址为0x4024b0+%rdx处的字符
  4010a0:	88 54 04 10          	mov    %dl,0x10(%rsp,%rax,1)  // 新的数据存储在 %rsp + 0x10 为起始地址的地方
  4010a4:	48 83 c0 01          	add    $0x1,%rax    // %rax + 1  （相当于遍历下一个字符
  4010a8:	48 83 f8 06          	cmp    $0x6,%rax    // 判断是否遍历到了结尾 
  4010ac:	75 dd                	jne    40108b <phase_5+0x29>  // 如果没有遍历结束 就继续循环
  4010ae:	c6 44 24 16 00       	movb   $0x0,0x16(%rsp)   // 给新生成的字符串后加上"\0"表示结尾
  4010b3:	be 5e 24 40 00       	mov    $0x40245e,%esi    // 传参
  4010b8:	48 8d 7c 24 10       	lea    0x10(%rsp),%rdi   // 新生成的字符串的起始地址
  4010bd:	e8 76 02 00 00       	callq  401338 <strings_not_equal>  // 判断新生成的字符串和对比字符串 是否相同
  4010c2:	85 c0                	test   %eax,%eax   // 查看返回结果是否为0
  4010c4:	74 13                	je     4010d9 <phase_5+0x77>  // %eax = 0 说明两个字符串相同
  4010c6:	e8 6f 03 00 00       	callq  40143a <explode_bomb>  // 如果两个字符串不相同就爆炸
 ...
 // phase_5的c代码
 char* array = "maduiersnfotvbylSo you think you can stop the bomb with ctrl-c, do you?";
 char* target_str = "flyers"  
 // array和target_str 都是bomb程序提前初始化好的内容
 void phase_5(const char* input){
   ...
   for(int i = 0; i<6; i++){
     int val = input[i];
     val = val % 16;     // 意味着只对array的前16个字符进行使用 也就是:maduiersnfotvbyl
     // 相当于把 输入字符 映射到 array中对应字符
     new_str[i] = array[val];
   }
   new_str[6] = '\0';
   int ret = strings_not_equal(new_str, target_str);
   if(ret != 0) explode_bomb();   // 如果new_str != target_str
   return;
 }
...
00000000004010f4 <phase_6>:
  ...
  //2. 读取数字
  401106:	e8 51 03 00 00       	callq  40145c <read_six_numbers>  // 读取6个数字 存在数组a中
  40110b:	49 89 e6             	mov    %rsp,%r14
  
  //3. 开始循环遍历 要求输入的数字在[1, 6]之间，且没有相等数字
  40110e:	41 bc 00 00 00 00    	mov    $0x0,%r12d                  // %r12 用于记录当前遍历到第几个数字i
  401114:	4c 89 ed             	mov    %r13,%rbp                   // %r13 用于保存当前遍历的数字的 地址a+i      
  401117:	41 8b 45 00          	mov    0x0(%r13),%eax              // 获取a[i]的值                               
  40111b:	83 e8 01             	sub    $0x1,%eax                   // 将该数字减1                                
  40111e:	83 f8 05             	cmp    $0x5,%eax                                                                 
  401121:	76 05                	jbe    401128 <phase_6+0x34>       // 如果该数字小于等于5就跳转（无符号）        
  401123:	e8 12 03 00 00       	callq  40143a <explode_bomb>       // 不然就爆炸                                 
  401128:	41 83 c4 01          	add    $0x1,%r12d                  // i + 1   准备遍历下一个数字                 
  40112c:	41 83 fc 06          	cmp    $0x6,%r12d                                                                
  401130:	74 21                	je     401153 <phase_6+0x5f>       // 判断是否等于6 （相当于判断是否遍历结束）   
  401132:	44 89 e3             	mov    %r12d,%ebx                                                                
  401135:	48 63 c3             	movslq %ebx,%rax                                                                 
  401138:	8b 04 84             	mov    (%rsp,%rax,4),%eax          // 获取下一个数字                             
  40113b:	39 45 00             	cmp    %eax,0x0(%rbp)              // 比较和第一个数字是否相等                   
  40113e:	75 05                	jne    401145 <phase_6+0x51>       //                                            
  401140:	e8 f5 02 00 00       	callq  40143a <explode_bomb>       // 相等就 爆炸 
  
  // 3. 子循环  
  401145:	83 c3 01             	add    $0x1,%ebx                   // j + 1                 
  401148:	83 fb 05             	cmp    $0x5,%ebx                                            
  40114b:	7e e8                	jle    401135 <phase_6+0x41>       // 判断j <= 5
  40114d:	49 83 c5 04          	add    $0x4,%r13                   // 相当于遍历下一个数 a+i
  401151:	eb c1                	jmp    401114 <phase_6+0x20>       // 循环执行
  
  // 4. 2 和 3的循环结束 进行下一步处理：7 - 输入的数字
  401153:	48 8d 74 24 18       	lea    0x18(%rsp),%rsi             // a存放的最后一个数字的下一个地址（结尾）  
  401158:	4c 89 f0             	mov    %r14,%rax                   // a的首地址放在%r14中 point = a            
  40115b:	b9 07 00 00 00       	mov    $0x7,%ecx                   // 将0x7 放在 %ecx中                        
  401160:	89 ca                	mov    %ecx,%edx                                                               
  401162:	2b 10                	sub    (%rax),%edx                                                             
  401164:	89 10                	mov    %edx,(%rax)                 // *point = 7 - *point 
  401166:	48 83 c0 04          	add    $0x4,%rax                   // point += 1 准备遍历下一个元素            
  40116a:	48 39 f0             	cmp    %rsi,%rax                   // point == 结尾？                          
  40116d:	75 f1                	jne    401160 <phase_6+0x6c>       // 如果不等于结尾就循环执行      
  40116f:	be 00 00 00 00       	mov    $0x0,%esi                   // i in %esi,  i = 0                        
  401174:	eb 21                	jmp    401197 <phase_6+0xa3>       // point等于结尾就跳转到phase_6+163执行 
  
  // 5. 4的循环结束 开始下一步的循环处理:使用b数组保存相应node节点的地址（该地址由a数组的元素进行映射）
  401176:	48 8b 52 08          	mov    0x8(%rdx),%rdx              // node* point = node->next;             
  40117a:	83 c0 01             	add    $0x1,%eax                   // tmp in %eax                           
  40117d:	39 c8                	cmp    %ecx,%eax                   // a[i] in %ecx, i in %rsi               
  40117f:	75 f5                	jne    401176 <phase_6+0x82>           
  401181:	eb 05                	jmp    401188 <phase_6+0x94>       
  401183:	ba d0 32 60 00       	mov    $0x6032d0,%edx               // 指针指向首地址                                       
  401188:	48 89 54 74 20       	mov    %rdx,0x20(%rsp,%rsi,2)       // 使用一个数组保存地址 b[i] = point                       
  40118d:	48 83 c6 04          	add    $0x4,%rsi                    // i++    
  401191:	48 83 fe 18          	cmp    $0x18,%rsi                                                   
  401195:	74 14                	je     4011ab <phase_6+0xb7>        // 如果i == 6 就结束该循环                                      
  401197:	8b 0c 34             	mov    (%rsp,%rsi,1),%ecx           // i in %rsi
  40119a:	83 f9 01             	cmp    $0x1,%ecx                     
  40119d:	7e e4                	jle    401183 <phase_6+0x8f>        // 如果a[i] <= 1                                
  40119f:	b8 01 00 00 00       	mov    $0x1,%eax                                                            
  4011a4:	ba d0 32 60 00       	mov    $0x6032d0,%edx                    
  4011a9:	eb cb                	jmp    401176 <phase_6+0x82>
  
  // 6. 5的循环操作结束，开始将链表按照b数组中存放的地址顺序 进行重新连接
  4011ab:	48 8b 5c 24 20       	mov    0x20(%rsp),%rbx
  4011b0:	48 8d 44 24 28       	lea    0x28(%rsp),%rax              // node** p = b+1的地址                  
  4011b5:	48 8d 74 24 50       	lea    0x50(%rsp),%rsi              // node** end = b+6的地址 （实际是b+6*8）
  4011ba:	48 89 d9             	mov    %rbx,%rcx                    // node* point1 = b[0]                   
  4011bd:	48 8b 10             	mov    (%rax),%rdx                  // node* point2 = *p                     
  4011c0:	48 89 51 08          	mov    %rdx,0x8(%rcx)               // point1->next = point2                 
  4011c4:	48 83 c0 08          	add    $0x8,%rax                    // p += 1;                               
  4011c8:	48 39 f0             	cmp    %rsi,%rax                    // ? p == end                            
  4011cb:	74 05                	je     4011d2 <phase_6+0xde>        // 判断是否遍历到了结尾                  
  4011cd:	48 89 d1             	mov    %rdx,%rcx                    // point1 = point2                       
  4011d0:	eb eb                	jmp    4011bd <phase_6+0xc9>
  
  // 7. 6的循环操作结束，要求连接之后的节点中存储的val 是 非递减的
  4011d2:	48 c7 42 08 00 00 00 	movq   $0x0,0x8(%rdx)               // point2->next = 0 结尾指针设为null           
  4011d9:	00                                                           
  4011da:	bd 05 00 00 00       	mov    $0x5,%ebp                    // 链表首地址存放在 %rbx  用point表示         
  4011df:	48 8b 43 08          	mov    0x8(%rbx),%rax               // %rax = point->next
  4011e3:	8b 00                	mov    (%rax),%eax                  // val = point->next->val                     
  4011e5:	39 03                	cmp    %eax,(%rbx)                  // 比较当前 节点和下一个节点中值的大小        
  4011e7:	7d 05                	jge    4011ee <phase_6+0xfa>                                                      
  4011e9:	e8 4c 02 00 00       	callq  40143a <explode_bomb>        // 如果当前节点的值 小于 下一个节点的值 就爆炸
  4011ee:	48 8b 5b 08          	mov    0x8(%rbx),%rbx               // point = point->next                        
  4011f2:	83 ed 01             	sub    $0x1,%ebp                    // %ebp - 1                                   
  4011f5:	75 e8                	jne    4011df <phase_6+0xeb>        // ? %ebp != 0 
 ...
 node* node1 = xxxx;   // node1记录了第一个节点的地址
 ​
 // phase_6的汇编有点绕，看c代码的话就比较清晰
 void phase_6(const char* input){
   ...
   int val = a[0] - 1;
   unsigned int tmp = 5;  
   // 操作2：要求输入的数字中没有相等的数字 并且在[1,6]之间
   int i = 0;
   while(true){
     /*
     unsigned int tmp = 5;
   ...
   // 操作4: 使用b数组重新对链表节点进行排序（依次存储链表节点的地址），
   // a[i]为多少, 就将第i+1个节点首地址，存储到b[i]中  (i从0开始)
   i = 0;
   node* b[6];
   node* point = null;
   for(int i = 0; i<6; i++){
     /*
     if(a[i] <= 1) point = node1;
     else{
       point = node1;
       int j = 1;
       do{
     ...
     */
     // 进行简化表示 node1是第一个节点的首地址
     point = node1;
     int j = 1;
     while(j++ < a[i]) point = point->next;
     b[i] = point;
   }
   
   // 操作5 按照b数组中存放顺序，重新进行链表的连接
   node* point1 = b[0];
   node* point2 = b[1];
   for(int i = 1; i<=5; i++){
     point2 = b[i];
   ...
   while(i--){
     point2 = point1->next;
     if(point1->val < point2->val) explode_bomb();
     point1 = point2;
   }
   return;
 }
...
0000000000401204 <fun7>:
  // 根节点root的地址放在%rdi，用户输入的数字num放在%rsi
  401204:	48 83 ec 08          	sub    $0x8,%rsp
  401208:	48 85 ff             	test   %rdi,%rdi
  40120b:	74 2b                	je     401238 <fun7+0x34>           // 根节点为空 就跳转到fun7+52   
  40120d:	8b 17                	mov    (%rdi),%edx                  // 根节点不为空                 
  40120f:	39 f2                	cmp    %esi,%edx                    // 比较根节点和输入数值num的大小
  401211:	7e 0d                	jle    401220 <fun7+0x1c>           // num <= root->val 才进行跳转  
  401213:	48 8b 7f 08          	mov    0x8(%rdi),%rdi               // root = root->left            
  401217:	e8 e8 ff ff ff       	callq  401204 <fun7>                // 递归调用                     
  40121c:	01 c0                	add    %eax,%eax                    // 返回值*2                     
  40121e:	eb 1d                	jmp    40123d <fun7+0x39>           // 返回结果                     
  401220:	b8 00 00 00 00       	mov    $0x0,%eax
  401225:	39 f2                	cmp    %esi,%edx
  401227:	74 14                	je     40123d <fun7+0x39>           // 如果num ==root->val 直接返回0 
  401229:	48 8b 7f 10          	mov    0x10(%rdi),%rdi              // root = root->right            
  40122d:	e8 d2 ff ff ff       	callq  401204 <fun7>                // 递归调用                      
  401232:	8d 44 00 01          	lea    0x1(%rax,%rax,1),%eax        // 返回值*2+1                    
  401236:	eb 05                	jmp    40123d <fun7+0x39>           // 返回结果                      
  401238:	b8 ff ff ff ff       	mov    $0xffffffff,%eax             // 返回值 = -1                   
  40123d:	48 83 c4 08          	add    $0x8,%rsp
  401241:	c3                   	retq  
  
 // 在二叉树中查找对应的num，然后依据查找路径和次数，返回相应的值
 int fun7(TreeNode* root, int num){
   if(root == null) return -1;
   if(root->val == num) return 0;
   if(num < root->val) return 2 * fun7(root->left, num);
...
0000000000401242 <secret_phase>:
  ...
  401255:	e8 76 f9 ff ff       	callq  400bd0 <strtol@plt>   // 将输入的字符串提取为整数 存在%rax  
  40125a:	48 89 c3             	mov    %rax,%rbx
  40125d:	8d 40 ff             	lea    -0x1(%rax),%eax    // 输入的数字 - 1 
  401260:	3d e8 03 00 00       	cmp    $0x3e8,%eax        // 要求%eax <= 1000（0x3e8）  
  401265:	76 05                	jbe    40126c <secret_phase+0x2a>   
  401267:	e8 ce 01 00 00       	callq  40143a <explode_bomb>    // 不然就爆炸
  // 2. 调用fun7
  40126c:	89 de                	mov    %ebx,%esi               // 输入的数字 作为第二个参数传入fun7 
  40126e:	bf f0 30 60 00       	mov    $0x6030f0,%edi          // 该内存地址作为 第一个参数传入fun7 
  401273:	e8 8c ff ff ff       	callq  401204 <fun7>           // 调用fun7函数                      
  401278:	83 f8 02             	cmp    $0x2,%eax
  40127b:	74 05                	je     401282 <secret_phase+0x40>     // fun7的返回结果为2说明成功 
  40127d:	e8 b8 01 00 00       	callq  40143a <explode_bomb>          // 不然爆炸                  
...
void secret_phase(){
   ...
   int num = strtol(input); // 字符串转数字
   if(num > 1001) explode_bomb();
   int ret = fun7(地址， num);
   if(ret != 2) explode_bomb();
   return;
 }
...
000000000040131b <string_length>:
  40131b:	80 3f 00             	cmpb   $0x0,(%rdi)    //str[0] == 0:判断该地址的首字符是不是结尾('\0')
  40131e:	74 12                	je     401332 <string_length+0x17>  // 如果是结尾，跳到401332处，直接返回0（即长度为0）
  401320:	48 89 fa             	mov    %rdi,%rdx
  401323:	48 83 c2 01          	add    $0x1,%rdx             // 相当于 current = str+1  
  401327:	89 d0                	mov    %edx,%eax
  401329:	29 f8                	sub    %edi,%eax             // 相当于 length = current - str 
  40132b:	80 3a 00             	cmpb   $0x0,(%rdx)           // 判断current是不是指向结尾了('\0') 
  40132e:	75 f3                	jne    401323 <string_length+0x8>    //如果没有指向结尾，继续循环，否则返回字符串长度
...
// strings_not_equal 汇编代码
// 用户输入的字符串（字符串1）地址在%rdi, bomb默认的字符串（字符串2）地址在%rsi
0000000000401338 <strings_not_equal>:
  401338:	41 54                	push   %r12
  40133a:	55                   	push   %rbp
  40133b:	53                   	push   %rbx
  //1. 首先比较字符串的长度是否相等
  40133c:	48 89 fb             	mov    %rdi,%rbx    // 提前将%rdi存起来(字符串1) 
  40133f:	48 89 f5             	mov    %rsi,%rbp    // 字符串2
  401342:	e8 d4 ff ff ff       	callq  40131b <string_length>   // 计算字符串1的长度
  401347:	41 89 c4             	mov    %eax,%r12d               // 结果保存在%r12
  40134a:	48 89 ef             	mov    %rbp,%rdi
  40134d:	e8 c9 ff ff ff       	callq  40131b <string_length>   // 计算字符串2的长度 
  401352:	ba 01 00 00 00       	mov    $0x1,%edx
  401357:	41 39 c4             	cmp    %eax,%r12d               // 比较两个字符串的长度是否相等
  40135a:	75 3f                	jne    40139b <strings_not_equal+0x63>     // 如果不相等直接跳转返回1
  //2. 判断长度是不是0
  40135c:	0f b6 03             	movzbl (%rbx),%eax     // 存放字符串1的第一个字符
  40135f:	84 c0                	test   %al,%al         // 判断第一个字符是不是结尾
  401361:	74 25                	je     401388 <strings_not_equal+0x50>   //如果是结尾 返回0
  //3. 比较字符串的内容是否相同 (判断第一个字符是不是结尾)
  401363:	3a 45 00             	cmp    0x0(%rbp),%al   //当前字符串1中字符(char1)和字符串2中字符(char2)是否相同
  401366:	74 0a                	je     401372 <strings_not_equal+0x3a>  // 如果char1和char2相同   
  401368:	eb 25                	jmp    40138f <strings_not_equal+0x57>  // 如果不相同返回1
  40136a:	3a 45 00             	cmp    0x0(%rbp),%al               // 比较char1 == char2 
  40136d:	0f 1f 00             	nopl   (%rax)
  401370:	75 24                	jne    401396 <strings_not_equal+0x5e>  // 如果char1 != char2 返回1
  401372:	48 83 c3 01          	add    $0x1,%rbx    // char1 += 1
  401376:	48 83 c5 01          	add    $0x1,%rbp    // char2 += 1
  40137a:	0f b6 03             	movzbl (%rbx),%eax
  40137d:	84 c0                	test   %al,%al      // 判断char1 == 0
  40137f:	75 e9                	jne    40136a <strings_not_equal+0x32>   // 如果char1 != 0 
  401381:	ba 00 00 00 00       	mov    $0x0,%edx                         // 如果char1 == 0 说明到结尾了 
...
// read_six_numbers 汇编代码
000000000040145c <read_six_numbers>:
  40145c:	48 83 ec 18          	sub    $0x18,%rsp   //分配24字节的栈帧
  401460:	48 89 f2             	mov    %rsi,%rdx        // %rsi + 0x0   num1的地址
  401463:	48 8d 4e 04          	lea    0x4(%rsi),%rcx   // %rsi + 0x4   num2的地址
  401467:	48 8d 46 14          	lea    0x14(%rsi),%rax  // %rsi + 0x14   num6的地址
  40146b:	48 89 44 24 08       	mov    %rax,0x8(%rsp)   // num6的地址没有寄存器存放了 将num6的地址临时存放在当前栈
  401470:	48 8d 46 10          	lea    0x10(%rsi),%rax  // %rsi + 0x10  num5的地址
  401474:	48 89 04 24          	mov    %rax,(%rsp)      // num5的地址没有寄存器存放了 将num5的地址临时存放在当前栈
  401478:	4c 8d 4e 0c          	lea    0xc(%rsi),%r9    // %rsi + 0xc   num4
  40147c:	4c 8d 46 08          	lea    0x8(%rsi),%r8    // %rsi + 0x8   num3
  401480:	be c3 25 40 00       	mov    $0x4025c3,%esi   // sscanf的接收格式 "%d %d %d %d %d %d" 
  401485:	b8 00 00 00 00       	mov    $0x0,%eax
  40148a:	e8 61 f7 ff ff       	callq  400bf0 <__isoc99_sscanf@plt>   // 调用sscanf 
  40148f:	83 f8 05             	cmp    $0x5,%eax        // %eax中存储的是输入数字的个数
  401492:	7f 05                	jg     401499 <read_six_numbers+0x3d>   // 输入数字的数目小于等于5 就是爆炸
  401494:	e8 a1 ff ff ff       	callq  40143a <explode_bomb>
  401499:	48 83 c4 18          	add    $0x18,%rsp
  40149d:	c3                   	retq   
  
 // read_six_numbers的c代码
 void read_six_numbers(const char* input, int* a){
     int len = sscanf(input, "%d %d %d %d %d %d", a, a+1, a+2, a+3, a+4, a+5);
     if(len <= 5) explode_bomb();
     return;
 }
...
00000000004015c4 <phase_defused>:
  ...
  4015df:	75 5e                	jne    40163f <phase_defused+0x7b>    // 如果当前没有拆完6个炸弹就跳转 
  4015e1:	4c 8d 44 24 10       	lea    0x10(%rsp),%r8                 // 已经拆完了6个炸弹，栈上分配第3个变量 num3
  4015e6:	48 8d 4c 24 0c       	lea    0xc(%rsp),%rcx                 // 栈上分配第2个变量 num2
  4015eb:	48 8d 54 24 08       	lea    0x8(%rsp),%rdx                 // 栈上分配第1个变量 num1                
  4015f0:	be 19 26 40 00       	mov    $0x402619,%esi                 // “%d %d %s”
  4015f5:	bf 70 38 60 00       	mov    $0x603870,%edi
  4015fa:	e8 f1 f5 ff ff       	callq  400bf0 <__isoc99_sscanf@plt>   // 对0x603870地址中的数据进行读取与解析
  4015ff:	83 f8 03             	cmp    $0x3,%eax                      // 判断是不是解析出三个参数
  401602:	75 31                	jne    401635 <phase_defused+0x71>    // 如果没有解析出3个参数 就触发不了秘密炸弹
  401604:	be 22 26 40 00       	mov    $0x402622,%esi                 // 用于比较的字符串s2
  401609:	48 8d 7c 24 10       	lea    0x10(%rsp),%rdi                // 解析出的第三个参数s1
  40160e:	e8 25 fd ff ff       	callq  401338 <strings_not_equal>     // 判断 s1 和 s2 是否相同
  401613:	85 c0                	test   %eax,%eax                      // strings_not_equal 返回值为0说明相同
  401615:	75 1e                	jne    401635 <phase_defused+0x71>    // 不相同 就无法触发秘密炸弹 
  401617:	bf f8 24 40 00       	mov    $0x4024f8,%edi                 // 该地址存放"Curses, you've found the secret phase!" 
  40161c:	e8 ef f4 ff ff       	callq  400b10 <puts@plt>              // 输出上述字符串 
  401621:	bf 20 25 40 00       	mov    $0x402520,%edi                 // 该地址存放"But finding it and solving it are quite different..."
  401626:	e8 e5 f4 ff ff       	callq  400b10 <puts@plt>              // 输出上述字符串
  40162b:	b8 00 00 00 00       	mov    $0x0,%eax
  401630:	e8 0d fc ff ff       	callq  401242 <secret_phase>          // 执行含有秘密炸弹的secret_phase函数
  401635:	bf 58 25 40 00       	mov    $0x402558,%edi                 // 该地址存放"Congratulations! You've defused the bomb!"
  40163a:	e8 d1 f4 ff ff       	callq  400b10 <puts@plt>              // 输出上述字符串 
 ...
 // 有一个全局变量记录我们在拆除第四个炸弹时的输入"7 0"
 char* phase_4_input = "7 0";
 char* target_str = "DrEvil";
 ​
 void phase_defused(){
   // num_input_strings 是一个全局变量，记录了当前拆除了多少个炸弹
   if(num_input_strings != 6) return;
   int num1, int num2;
   char* str;
   int len = sscanf(phase_4_input, "%d %d %s", &num1, %num2, str);
   if(len != 3) puts("Congratulations! You've defused the bomb!");
   else{
     // 只有当str == "DrEvil"时才能成功触发 进入secret_phase
     int ret = strings_not_equal(str, target_str);
     if(ret == 0)  secret_phase();
   }
