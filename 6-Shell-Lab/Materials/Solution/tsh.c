...
void eval(char *cmdline) 
{
    ...
    if(!builtin_cmd(argv)) {                            //判断是否为内置命令
        Sigprocmask(SIG_BLOCK, &mask, &prev_mask);       //fork前阻塞SIGCHLD信号
        if((pid = Fork()) == 0) {                           //创建子进程
            Sigprocmask(SIG_SETMASK, &prev_mask, NULL);      //解除子进程的阻塞
...
void sigint_handler(int sig) 
{
    ...
    sigset_t mask_all, prev_mask;
    Sigfillset(&mask_all);
    Sigprocmask(SIG_BLOCK, &mask_all, &prev_mask);   //jobs为全局变量
    if((pid = fgpid(jobs)) != 0){
        Sigprocmask(SIG_SETMASK, &prev_mask, NULL);
        Kill(-pid, SIGINT);
    }
